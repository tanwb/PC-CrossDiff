...
class SpacyParser(ParserBackend):
    ...
    def parse(self, sentence, return_doc=False):
        ...
        for entity in doc.noun_chunks:
            ...
            # ent['modifiers'].sort(key=lambda m: m['idx'])
        ...
        for idx, ec in enumerate(entity_chunks):
            ...
            if (ec.root.lemma_ in self.obj_cls) or (database.is_object_noun(ec.root.lemma_)) or \
                    (entities[idx]['lemma_head'] in self.obj_cls) or (
            database.is_object_noun(entities[idx]['lemma_head'])) or \
                    (True in [(doc[i].lemma_ in self.obj_cls) or (database.is_object_noun(doc[i].lemma_)) for i in
                              head_id_list]):
        ...
        for idx_s, entity in enumerate(filter_entity_chunks):
            ...
            if (root_token != None) and (root_token.dep_ in ['ROOT', 'ccomp']):
                for idx_o, obj_ent in enumerate(filter_entity_chunks):
                    if (obj_ent.root.head == root_token) \
                        ...
                        rel, rel_idx, token_mask = self.__find_rel_by_root(root_token, entity, filter_entity_chunks,
                                                                           doc, token_mask)
            ...
            # eg. it is sitting on carpet and there [is] a black [chair] to the right of [it].
                ...
                for idx_o, obj_ent in enumerate(filter_entity_chunks):
                    if (obj_ent.root.head == root_conj) \
                        ...
                        rel, rel_idx, token_mask = self.__find_rel_by_root(root_conj, entity, filter_entity_chunks, doc,
                                                                           token_mask)
            ...
            # step 2. The sentence is not the main clause, if no ROOT is found.
                ...
                rel, rel_idx, token_mask = self.__find_rel_by_entity(root_entity, entity, filter_entity_chunks, doc,
                                                                     token_mask)
            ...
            if (root_token != None) and (relation == None):
                if (root_token.i > 0) and (doc[root_token.i - 1].head != root_token) and (root_token.tag_ == 'VBN'):
                    rel, rel_idx, token_mask = self.__find_rel_by_root(root_token, entity, filter_entity_chunks, doc,
                                                                       token_mask)
                    relation = {
                        'obj_idx': 0,
        ...
        for idx, entity in enumerate(filter_entity_chunks):
            ...
            if len(i_list) > 1:
                for i in i_list:
                    x = doc[i]
                    filter_entities[idx]['modifiers'].append(
                        {'dep': x.dep_, 'span': x.text, 'idx': x.i, 'lemma_span': x.lemma_}) \
 \
        ...
        for idx, node in enumerate(nodes):
            ...
            # 加属性目标，在这里返回“描述对象及其修饰属性”，如“quick brown fox”，当输入为 "The quick brown fox jumps over the lazy dog."时
        # modified_entity= +" "+ main_entity

        # # 找到主实体对应的节点
        # main_node = next((node for node in nodes if node['node_id'] == 0), None)
        # if main_node:
        #     # 合并修饰符和核心词，例如 ['quick', 'brown'] + ['fox'] -> 'quick brown fox'
        #     modifiers = main_node['mod_text']
        #     head = main_node['target']
        #     modified_entity = ' '.join(modifiers + [head])
        # else:
        #     modified_entity = ''
        ...
        main_node = None
        for node in nodes:
            if node.get('node_id', -1) == 0:  # 默认值 -1 防止无 node_id 的情况
                main_node = node
                break

        # 生成修饰短语
        modified_entity = main_entity # 默认
        if main_node:
            # 安全获取字段
            modifiers = main_node.get('mod_text', [])
            head = main_node.get('target', '')
            # 过滤空值并拼接
            valid_mods = [m for m in modifiers if m.strip()]
            modified_entity = ' '.join(valid_mods + [head]).strip()


        return nodes, graph_dege, modified_entity
        # return nodes, graph_dege,modified_entity
