...
# ------------------------------------------------------------------------
from .pc_cross_diff_attention import MultiheadAttention
import numpy as np
import torch
...
class PC_CrossDiff(nn.Module):
    ...
    def __init__(self, num_class=256, num_obj_class=485,
        ...
        self.bert_mask_cross_learn=clda(d_model=288)
    ...
    def _run_backbones(self, inputs):
        ...
        encoded_text = self.text_encoder(**tokenized).last_hidden_state # [b,x,768]
        text_feats = self.text_projector(encoded_text)  # ([B, L ,288])
        text_mask = tokenized.attention_mask.ne(1).bool()

        attr_list=inputs['modified_entity_bs']
        text = inputs['text']
        for i in range(len(attr_list)):
            if attr_list[i]=='' or len(str(attr_list[i])) < 3 :
                attr_list[i]=text[i]



        # print("\ntext:", inputs['text'])
        # print("attr_obj:",inputs['modified_entity_bs'],"\n")
        attribute_object_tokenized = self.tokenizer.batch_encode_plus(
            attr_list, padding="longest", return_tensors="pt"
        ).to(inputs['point_clouds'].device)
        attr_obj_feats = self.text_encoder(**attribute_object_tokenized).last_hidden_state  # [b,x,768]


        end_points = self.backbone_net(point_clouds=inputs['point_clouds'],
                                       text_feats=encoded_text,
                                       attr_obj_feats=attr_obj_feats,
                                      )  # 50000 points -> 1024 points        # end_points = self.backbone_net(inputs['point_clouds'], end_points={}) # 50000 points -> 1024 points
        ...
        end_points['text_attention_mask'] = text_mask
    ...
    def forward(self, inputs):
        ...
        points_features, text_feats, text_padding_mask = self.bert_mask_cross_learn(
            vis_feats=points_features,
            points_xyz=points_xyz.contiguous(),  # original xyz ([B, 1024, 3])
            ...
            spatial_point_xyz=None,
            superpoint=inputs['superpoint'],
            source_xzy=inputs['point_clouds'][..., 0:3].contiguous()
        )

        # print(f"Parameters of self.bert_mask_cross_learn: {sum(p.numel() for p in self.bert_mask_cross_learn.parameters())}")


        end_points['text_attention_mask'] = text_padding_mask  # text mask has already been updated
        ...
        for bs in range(source_xzy.shape[0]):
            super_xyz = scatter_mean(source_xzy[bs], superpoint[bs], dim=0).unsqueeze(
                0)  # [1, super_num, 3]  compute the mean coordinate of each superpoint to obtain its center
            super_xyz_list.append(super_xyz)
            grouped_feature, ball_idx = self.super_grouper(points_xyz[bs].unsqueeze(0), super_xyz,
                                                           mask_feats[bs].unsqueeze(0))  # [1, 288, super_num, nsample]
            ...
            super_feature = F.max_pool2d(grouped_feature, kernel_size=[1, grouped_feature.size(3)]).squeeze(-1).squeeze(
                0)  # [288, super_num]
            super_feature = F.normalize(super_feature, p=2, dim=0)
        ...
        for bs in range(query.shape[0]):
            ...
            for i in range(3):
                bs_text_query, _, src_weight = self.swa_layers[i](
                    super_features[bs].unsqueeze(0).transpose(1, 2).transpose(0, 1), bs_text_query.transpose(0, 1),
                    attn_mask=attn_masks)  # SWA module
                bs_text_query = self.swa_ffn_layers[i](bs_text_query)
                _, _, attn_masks = self.prediction_head(bs_text_query, super_features[bs].unsqueeze(0).transpose(1, 2))
            src_weight = src_weight.softmax(1)
            src_weight = torch.where(torch.isnan(src_weight), torch.zeros_like(src_weight),
                                     src_weight)  # replace NaNs in src_weight with 0
            q_score = (src_weight * ~text_padding_mask[bs].unsqueeze(-1)).sum(-1)  # [B, N_q]
            q_idx = q_score[0].argmax(dim=-1, keepdim=True)
            pred_scores, pred_masks, _ = self.prediction_head(bs_text_query[:, q_idx, :],
                                                              super_features[bs].unsqueeze(0).transpose(1, 2))
            prediction_masks.append(pred_masks.expand(1, 256, pred_masks.shape[-1]))  # append the per-batch list to the global list
