...
from models import PC_CrossDiff  # Resolved via args.model; not called explicitly here (used around line 101).
...
class TrainTester(BaseTrainTester):
    ...
    def get_datasets(args):
        ...
        if args.eval:
            ...
            train_dataset = Joint3DDataset(
                ...
                parse_result_dir=args.parse_result_dir
            )
    ...
    def get_model(args):
        ...
        if model_name == 'BeaUTyDETR':
            model_name = 'PC_CrossDiff'
        if model_name in globals():
            ModelClass = globals()[model_name]
        else:
            raise ValueError(f"Unknown model '{args.model}'. Allowed: PC_CrossDiff")
        model = ModelClass(
            num_class=num_class,
            num_obj_class=485,
    ...
    def _get_inputs(batch_data):
        return {
            ...
            # 'attr_obj_relations': batch_data['attr_obj_relations'],
            ...
            # Attribute-target placeholderbs
            , 'modified_entity_bs': batch_data['modified_entity_bs']
        }

    # BRIEF eval without computing loss
    @torch.no_grad()
    ...
    def evaluate_one_epoch(self, epoch, test_loader, model, args):
        ...
        for batch_idx, batch_data in enumerate(test_loader):
            ...
            stat_dict, end_points = self._main_eval_branch(
                batch_idx, batch_data, test_loader, model, stat_dict, args
            )
        ...
        do_sync = dist.is_initialized() and dist.get_world_size() > 1
        if do_sync:
        ...
        if dist.get_rank() == 0:
            if evaluator is not None:
                ...
                det_50, _ = evaluator._overall_det("50")
                seg_50, _ = evaluator._overall_mask("50")
                score = float((s_50 + c_50) * 0.5)
                self._latest_eval_stats = {
                    "legacy_main_score": float(score),
                    "main_score": float(score),
                    "det_score": float(det_50),
                    "seg_score": float(seg_50),
                    "det_seg_gap": float(abs(det_50 - seg_50)),
                    "soft_token_50": float(s_50),
                    "contrastive_50": float(c_50),
                }
    ...
    @torch.no_grad()
    def evaluate_one_epoch_det(self, epoch, test_loader, model, args):
        """
        Eval grounding after a single epoch.
        ...
        if dist.is_initialized() and dist.get_world_size() > 1:
            ...
            dist.all_gather_object(gathered_pred, batch_pred_map_cls_dict)
            dist.all_gather_object(gathered_gt, batch_gt_map_cls_dict)
            merged_pred = {k: [] for k in prefixes}
            merged_gt = {k: [] for k in prefixes}
            for rank_pred, rank_gt in zip(gathered_pred, gathered_gt):
                for prefix in prefixes:
                    merged_pred[prefix].extend(rank_pred.get(prefix, []))
                    merged_gt[prefix].extend(rank_gt.get(prefix, []))
            batch_pred_map_cls_dict = merged_pred
            batch_gt_map_cls_dict = merged_gt
...
def _run_worker(local_rank, opt):
    opt.local_rank = int(local_rank)
    ...
    try:
        train_tester.main(opt)
    finally:
        if dist.is_initialized():
            dist.destroy_process_group()


def _spawn_worker(local_rank, world_size, opt):
    os.environ["LOCAL_RANK"] = str(local_rank)
    os.environ["RANK"] = str(local_rank)
    os.environ["WORLD_SIZE"] = str(world_size)
    _run_worker(local_rank, opt)
...
if __name__ == '__main__':
    ...
    env_local_rank = int(os.environ.get("LOCAL_RANK", opt.local_rank))

    if env_world_size > 1:
        _run_worker(env_local_rank, opt)
    else:
        visible_gpu_num = torch.cuda.device_count()
        os.environ.setdefault("MASTER_ADDR", "127.0.0.1")
        os.environ.setdefault("MASTER_PORT", "29500")
        if visible_gpu_num > 1:
            mp.spawn(_spawn_worker, nprocs=visible_gpu_num, args=(visible_gpu_num, opt))
        else:
