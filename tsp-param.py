    ...
    def get_datasets(args):
        ...
        if args.eval:
            ...
            train_dataset = Joint3DDataset(
                ...
                parse_result_dir=getattr(args, 'script_log_dir', args.log_dir)
            )
            ...
            if evaluator is not None:
                ...
                for t in evaluator.thresholds:
                    self.logger.info(''.join([
                        f"{'3dcnn'} Acc{t:.2f}: ",
                        f"Top-{1}: {evaluator.dets[('3dcnn', t, 1, 'bbf')] / max(evaluator.gts[('3dcnn', t, 1, 'bbf')], 1):.5f}"
                    ]))

        print('inf: ', np.array(inf_speeds).mean(), 'vis_back_speeds: ', np.array(vis_back_speeds).mean(),
              'text_back_speeds: ', np.array(text_back_speeds).mean(), 'fuiosn_speeds: ',
              np.array(fuiosn_speeds).mean(),
