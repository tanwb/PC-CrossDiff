...
from .visual_data_handlers import Scan
from .scannet_classes import REL_ALIASES, VIEW_DEP_RELS
# from models.attribute_object_extract import extract_subject_and_attributes
...
class Joint3DDataset(Dataset):
    ...
    def __init__(self, dataset_dict={'sr3d': 1, 'scannet': 10},
                 ...
                 wo_obj_name="None", parse_result_dir=None):
        ...
        self.parse_result_dir = parse_result_dir or os.path.join(os.getcwd(), 'parse_result')
        os.makedirs(self.parse_result_dir, exist_ok=True)
        ...
        if self.split != 'train':
            ...
            # file_name = 'scanrefer_val_preprocessed_data.pkl'  # Create a filename that starts with the dataset name
            # file_name = 'sr3d_val_preprocessed_data.pkl'
            # with open(file_name, 'wb') as file:
            #     pickle.dump(self.annos, file)  # Save data to file
            ...
            for dset, cnt in dataset_dict.items():
                if cnt > 0:
                    ...
                    # file_name = f'{dset}_preprocessed_data.pkl'  # Create a filename that starts with the dataset name
                    # with open(file_name, 'wb') as file:
                    #     pickle.dump(_annos, file)  # Save data to file
                    # with open(file_name, 'rb') as file:
    ...
    def load_sr3d_annos(self, dset='sr3d'):
        ...
        data_file = os.path.join(self.data_path, 'ReferIt3D', f'{dset}.csv')
        with open(data_file) as f:
            ...
            annos = [
                {
                    ...
                    # Attribute-target placeholder: initialize from utterance and let Scene_graph_parse generate the final modified_entity
                    'modified_entity': line[headers['utterance']],
            ...
            # text decoupling with cache
            cache_tag = _build_cache_tag(dset, self.split, data_file)
            Scene_graph_parse(annos, cache_tag=cache_tag, split=self.split, parse_result_dir=self.parse_result_dir)
    ...
    def load_nr3d_annos(self):
        ...
        with open('data/meta_data/nr3d_%s_scans.txt' % split) as f:
            scan_ids = set(eval(f.read()))
        data_file = os.path.join(self.data_path, 'ReferIt3D', 'nr3d.csv')
        with open(data_file) as f:
            csv_reader = csv.reader(f)
            headers = next(csv_reader)
            headers = {header: h for h, header in enumerate(headers)}
        ...
        cache_tag = _build_cache_tag("nr3d", self.split, data_file)
        Scene_graph_parse(annos, cache_tag=cache_tag, split=self.split, parse_result_dir=self.parse_result_dir)
    ...
    def load_scanrefer_annos(self):
        ...
        annos_path = _path + '_%s.json' % split
        with open(annos_path) as f:
            reader = json.load(f)
        if self.wo_obj_name != "None":
            annos_path = self.wo_obj_name
            with open(annos_path) as f:
                reader = json.load(f)
        ...
        annos = [
            {
                ...
                # 'attr_obj_relations': str(anno['relation']),  # placeholder field
                # 'description': str(anno['description']),  # placeholder field

                # Attribute-target placeholder; initialize it with a dummy slot
                'modified_entity': ' '.join(anno['token']),
        ...
        ###########################
        cache_tag = _build_cache_tag("scanrefer", self.split, annos_path)
        Scene_graph_parse(annos, cache_tag=cache_tag, split=self.split, parse_result_dir=self.parse_result_dir)
        ...
        # print(f"# STEP 4. Add unique-multi\n{annos}\n")
        return annos
    ...
    def load_scannet_annos(self):
        ...
        for scan_id in scan_ids:
            ...
            if keep.any():
                ...
                # If randomization is disabled, replace this with a fixed value
                ...
                annos.append({
                    ...
                    # 'attr_obj_relations': '',  # Placeholder field for randomized prompts; keep it when randomization is enabled
                    # 'description': '',  # Placeholder field for randomized prompts; keep it when randomization is enabled
...
#             'attr_obj_relations': anno['attr_obj_relations'],  # Not populated by default; load_scannet_annos() must add an empty field first
#             'description': anno['description'],  # Not populated by default; load_scannet_annos() must add an empty field first

            # Attribute-target placeholder bs
            "modified_entity_bs": (
                ' '.join(anno['modified_entity'].replace(',', ' ,').split())
            ),

            # "attribute_object": extract_subject_and_attributes(anno['utterance']),
...
#########################
def _ensure_parse_fields(annos):
    for anno in annos:
        if 'graph_node' not in anno:
            anno['graph_node'] = []
        if 'graph_edge' not in anno:
            anno['graph_edge'] = []
        if 'auxi_entity' not in anno:
            anno['auxi_entity'] = None
        if 'modified_entity' not in anno:
            anno['modified_entity'] = None


def _build_cache_signature(annos):
    hasher = hashlib.sha1()
    for anno in annos:
        hasher.update(str(anno.get('scan_id', '')).encode('utf-8'))
        hasher.update(b'\t')
        hasher.update(str(anno.get('target_id', '')).encode('utf-8'))
        hasher.update(b'\t')
        hasher.update(str(anno.get('utterance', '')).encode('utf-8'))
        hasher.update(b'\n')
    return hasher.hexdigest()


def _build_cache_tag(base_tag, split, source_path=None):
    cache_id = "unknown"
    if source_path:
        base_name = os.path.basename(source_path)
        try:
            stat = os.stat(source_path)
            cache_id = f"{base_name}_{int(stat.st_mtime)}_{stat.st_size}"
        except OSError:
            cache_id = f"{base_name}_missing"
    return f"{base_tag}_{split}_{cache_id}"


def _preprocess_caption(caption, dataset):
...
def Scene_graph_parse(annos, cache_tag=None, skip_if_test=False, split='train', parse_result_dir=None):
    if skip_if_test and split != 'train':
        _ensure_parse_fields(annos)
        return annos

    if parse_result_dir:
        os.makedirs(parse_result_dir, exist_ok=True)

    cache_signature = _build_cache_signature(annos)
    cache_path = None
    if cache_tag and parse_result_dir:
        cache_path = os.path.join(parse_result_dir, f"{cache_tag}.pkl")

    def _apply_cached(parsed_entries):
        for anno, parsed in zip(annos, parsed_entries):
            anno['utterance'] = parsed.get('utterance', anno.get('utterance', ''))
            anno['graph_node'] = parsed.get('graph_node', [])
            anno['graph_edge'] = parsed.get('graph_edge', [])
            anno['auxi_entity'] = parsed.get('auxi_entity', None)
            if 'modified_entity' in parsed:
                anno['modified_entity'] = parsed.get('modified_entity', None)

    if cache_path and os.path.exists(cache_path):
        logger.info(f"[Scene_graph_parse] Loaded from cache: {cache_path}")
        try:
            with open(cache_path, 'rb') as f:
                cached = pickle.load(f)

            if not isinstance(cached, dict) or 'entries' not in cached:
                logger.warning("Cache format is outdated or incomplete; removing cache and reparsing")
                os.remove(cache_path)
            else:
                if cached.get('version') != PARSE_CACHE_VERSION:
                    logger.warning("Cache version mismatch; removing cache and reparsing")
                    os.remove(cache_path)
                elif cached.get('signature') != cache_signature:
                    logger.warning("Cache signature mismatch; removing cache and reparsing")
                    os.remove(cache_path)
                else:
                    cached_entries = cached.get('entries', [])
                    if len(cached_entries) != len(annos):
                        logger.warning("Cache length mismatch; removing cache and reparsing")
                        os.remove(cache_path)
                    elif not all(isinstance(item, dict) for item in cached_entries):
                        logger.warning("Cache content structure is invalid; removing cache and reparsing")
                        os.remove(cache_path)
                    else:
                        _apply_cached(cached_entries)
                        logger.info(f"[Scene_graph_parse] Cache hit; reusing {len(annos)} parsed entries")
                        return annos
    ...
    for anno in progress:
        ...
        with _sng_parser_lock:
            graph_node, graph_edge, modified_entity = sng_parser.parse(caption)
        ...
        anno['modified_entity'] = modified_entity
    ...
    if cache_path:
        try:
            ...
            if reloaded.get('signature') != cache_signature or len(reloaded.get('entries', [])) != len(annos):
                raise ValueError("Cache write verification failed")

            os.replace(tmp_path, cache_path)
            logger.info(f"[Scene_graph_parse] Saved cache to {cache_path} ({len(annos)} entries)")
        except Exception as e:
            logger.warning(f"[Scene_graph_parse] Failed to save cache: {e}")
            try:
