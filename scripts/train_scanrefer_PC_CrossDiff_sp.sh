...
TORCH_DISTRIBUTED_DEBUG=INFO CUDA_VISIBLE_DEVICES=4,5,6,7 python -m torch.distributed.launch \
    --nproc_per_node 4 --master_port 4453 \
    ...
    --model PC_CrossDiff \
    --exp "PC_CrossDiff" \
    --checkpoint_path  none # put the resume checkpoint path here
