...
TORCH_DISTRIBUTED_DEBUG=INFO CUDA_VISIBLE_DEVICES=2,3,4,5,6,7 python -m torch.distributed.launch \
    --nproc_per_node 6 --master_port 4453 \
    ...
    --max_epoch 150 \
    --dgt_start_epoch 53 --dgt_stop_epoch 66 \
    ...
    --geom_late_start_epoch 72 --geom_gap_threshold 0.052 --geom_gap_worsen_threshold 0.004 --geom_quality_floor 0.40 --geom_seg_stable_drop 0.006 \
    --late_stage_confirm_epochs 2 --late_stage_cooldown_epochs 2 --late_stage_kill_drop 0.0025 \
        --enable_grad_consistency \
    --model PC_CrossDiff \
    --exp "PC_CrossDiff" \
    --checkpoint_path  none #/trxydsjtwb/data/model/vg3d/all_logs/PC_CrossDiff/scanrefer/1780757482/ckpt_epoch_70.pth #/trxydsjtwb/data/model/vg3d/all_logs/PC_CrossDiff/scanrefer/1780564709/ckpt_epoch_29.pth #none # put the resume checkpoint path here

LATEST_RUN_DIR="$(find "${LOG_ROOT}/PC_CrossDiff/scanrefer" -mindepth 1 -maxdepth 1 -type d | sort | tail -n 1)"
if [ -n "${LATEST_RUN_DIR}" ]; then
    echo "[done] latest run dir: ${LATEST_RUN_DIR}"
    echo "[done] best checkpoint: ${LATEST_RUN_DIR}/ckpt_best.pth"
    echo "[done] last checkpoint: ${LATEST_RUN_DIR}/ckpt_epoch_last.pth"
fi
